//
//  CastSelectViewController2.swift
//  swift_skyway
//
...
class CastSelectViewController2: UIViewController,UIScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,IndicatorInfoProvider {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if(self.appDelegate.castMoviePlayerNumSubMenu01 == 1){
            self.player00.play()
        }else if(self.appDelegate.castMoviePlayerNumSubMenu01 == 2){
            self.player01.play()
        }else if(self.appDelegate.castMoviePlayerNumSubMenu01 == 3){
            self.player02.play()
        }
        //初期状態に戻す
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        /*********************************************************/
        if(castList[indexPath.row].cast_official_flg == "3"){
            //待機状態の帯
        ...
        /*********************************************************/
        //自動映像流すアカウント(cast_official_flg = 3)は必ず待機状態にする(ここまで)
        /*********************************************************/
        ...
        /*********************************************************/
        //自動映像流すアカウント(cast_official_flg = 3)
        /*********************************************************/
        if(castList[indexPath.row].cast_official_flg == "3"){
            //ライブ視聴ボタンを押した時の処理
            testCell.liveStartBtn.tag = indexPath.row
            testCell.liveStartBtn.addTarget(self, action: #selector(liveStartRequestAuto(sender: )), for: .touchUpInside)
        }else{
            //ライブ視聴ボタンを押した時の処理
            testCell.liveStartBtn.tag = indexPath.row
    ...
    //予約なしのバージョン(自動応答ユーザーへの接続時)
    //不要な処理が多くあるが、取り急ぎ通常ストリーマーの処理をほぼコピー
    func setDialogAuto(cast_id:Int, cast_name:String){
        var strLiveTime = ""
        ...
        if(self.appDelegate.request_password == "0" || self.appDelegate.request_password == ""){
            ...
            let modifyAction = UIAlertAction(title: "サシライブする", style: UIAlertAction.Style.default, handler: {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
                    ...
                    UtilToViewController.toMediaConnectionViewControllerAuto()//自動応答ユーザー用
                }
    ...
    /////////追加(ここから)////////////////////////////
    // ライブ視聴ボタンが押されたら呼ばれる(自動応答ユーザーの場合)
    //func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    @objc func liveStartRequestAuto(sender: UIButton){
        ...
        // 全ての非同期処理完了後にメインスレッドで処理
        dispatchGroup.notify(queue: .main) {
            if(self.strSelectLoginStatus == "1"){
                //0:待機中
                //1:現在配信中で、予約はなし
                //2以上:現在配信中で、予約もあり
                ...
                //1回の視聴で消費するコイン
                self.appDelegate.live_pay_coin = 0

                self.setDialogAuto(
                    cast_id:Int(self.strSelectUserId)!,
                    cast_name:self.strSelectUserName)
            }
        }
    }
