                ...
                conditionRef.observeSingleEvent(of: .value, with: { (snapshot) in
                    ...
                    if(snapshot.exists() == true){
                        ...
                        self.handle_status = self.conditionRef.child("status").observe(.value) { (snap: DataSnapshot) in
                            ...
                            if(status == 99){
                                //承認された(キャストと繋がりました!と1秒ほど表示)
                                self.conditionRef.removeObserver(withHandle: self.handle_status)

                                // NewSDK: read room_name from full Firebase document
                                self.conditionRef.observeSingleEvent(of: .value, with: { snap in
                                    let roomName = snap.childSnapshot(forPath: "room_name").value as? String ?? ""
                                    print("[NewSDK][LISTENER] status=99 received room_name=\(roomName) cast_id=\(cast_id) user_id=\(user_id)")
