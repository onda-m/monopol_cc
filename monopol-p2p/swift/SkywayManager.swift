...
//  Copyright ﾂｩ 2018蟷ｴ worldtrip. All rights reserved.
//
...
class SkywayManager: NSObject, RoomDelegate, LocalRoomMemberDelegate, RoomPublicationDelegate, RoomSubscriptionDelegate {
    ...
    //API Key
    static let apiKey: String = "<縺ゅ↑縺溘・ID>"

    //Domain
    static let domain: String = "<縺ゅ↑縺溘・謖・ｮ壹＠縺歸omain>"
    ...
    /// 繝ｫ繝ｼ繝縺ｫ蜿ょ刈縺励√Ο繝ｼ繧ｫ繝ｫ繧ｹ繝医Μ繝ｼ繝繧・publish 縺吶ｋ
    /// - Parameters:
    ///   - roomName: 蜿ょ刈縺吶ｋ繝ｫ繝ｼ繝蜷搾ｼ医く繝｣繧ｹ繝医・蝣ｴ蜷医・閾ｪ蛻・・ peerId縲√Θ繝ｼ繧ｶ繝ｼ縺ｮ蝣ｴ蜷医・繧ｭ繝｣繧ｹ繝医・ peerId・・
    ///   - delegate: 繧ｳ繝ｼ繝ｫ繝舌ャ繧ｯ蜿嶺ｿ｡逕ｨ繝・Μ繧ｲ繝ｼ繝・
    public func connectStart(roomName: String, delegate: SkywaySessionDelegate) {
        ...
        // 螟夐㍾髢句ｧ九ぎ繝ｼ繝・ 譌｢縺ｫ謗･邯壻ｸｭ縺ｮ蝣ｴ蜷医・繧ｹ繧ｭ繝・・
        ...
        sessionDelegate = delegate
        roomClosed = false
        subscribedPublicationIds.removeAll()  // 繝ｪ繧ｻ繝・ヨ
        roomTask?.cancel()
    ...
    private func detachRoomCallbacks() async {
        ...
        roomSubscriptions.removeAll()  // 蠢・・ 莠碁㍾蜃ｦ逅・亟豁｢
        ...
        roomPublications.removeAll()  // 蠢・・ 莠碁㍾蜃ｦ逅・亟豁｢
        ...
        // (i) Detach local views first, then clear local streams (蜀肴磁邯壽凾縺ｯ prepareLocalStreamsIfNeeded 縺ｧ蜀咲函謌・
        detachLocalVideo()
        localVideoStream = nil
        localAudioStream = nil
        localDataStream = nil
    ...
    func roomDidClose(_ room: Room) {
        Task { @MainActor in
            ...
            await self.detachRoomCallbacks()  // isConnectStarted = false 縺ｯ縺薙％縺ｧ險ｭ螳壹＆繧後ｋ
    ...
    func room(_ room: Room, memberDidLeave member: RoomMember) {
        Task { @MainActor in
            guard self.delegatesAttached else { return }
            guard !self.roomClosed else { return }  // 譌｢縺ｫ髢峨§縺ｦ縺・ｋ蝣ｴ蜷医・繧ｹ繧ｭ繝・・
            guard let localMember = self.localMember else { return }
            if self.memberIdentifier(member) != self.localMemberIdentifier(localMember) {
                print("[SkyMgr] memberDidLeave: remote member left, memberId=\(member.id)")
                // 1蟇ｾ1騾夊ｩｱ縺ｪ縺ｮ縺ｧ逶ｸ謇九′騾蜃ｺ縺励◆繧臥ｵゆｺ・桶縺・
                self.sessionDelegate?.connectDisconnect()
                // 繝ｫ繝ｼ繝縺九ｉ騾蜃ｺ縺励※繝ｪ繧ｽ繝ｼ繧ｹ繧偵け繝ｪ繝ｼ繝ｳ繧｢繝・・
                await self.leaveRoomIfNeeded()  // isConnectStarted = false 縺ｯ縺薙％縺ｧ險ｭ螳壹＆繧後ｋ
            }
        }
    }
